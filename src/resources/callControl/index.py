...
chime_client = boto3.client("chime", config=client_config, region_name=os.environ["AWS_REGION"])
...
def disable_forward(body, queried_number):
    ...
    if queried_number["PhoneNumber"]["ProductType"] == "VoiceConnector":
        return "200", json.dumps(
            "{} is already a Voice Connector product type".format(body["PhoneNumber"]), default=json_serial
        )
    else:
...
def ddb_remove_forward(from_number):
    try:
        ...
        logger.error("DynamoDB Query error: failed to delete data from table. Error: ", exc_info=err)
        return None
