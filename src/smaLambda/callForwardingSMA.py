...
def lambda_handler(event, context):
    ...
    if event_type == 'NEW_INBOUND_CALL':
        logger.info('RECV {} {}'.format(
            log_prefix, 'New inbound call initiated'))
        ...
        logger.info('RECV {} {}'.format(log_prefix, 'Rining event received'))
        return ()
    ...
    if destination_number:
        ...
        if loop_flag == 'True':
            ...
            logger.info('SEND {} {}'.format(
                log_prefix, 'Sending CallAndBridge action with greeting ringback'))
            ...
            logger.info('SEND {} {}'.format(
                log_prefix, 'Sending PlayAudio, CallAndBridge actions'))
        ...
        logger.info('NONE {} {}'.format(
            log_prefix, 'No entry found in database - sending hangup'))
        return unable_to_connect(call_id)
...
def ddb_get_destination(to_number):
    try:
        ...
        logger.error(
            'DynamoDB Query error: failed to fetch data from table. Error: ', exc_info=err)
        return None
