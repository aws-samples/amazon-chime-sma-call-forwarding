...
def get_phone_number():
    ...
    while not order_status == 'Successful':
        ...
        if order_status == 'Failed':
            raise Exception(
                'Order number failed: {}'.format(check_phone_order))
        if timeout == 5:
...
def create_voice_connector(region):
    ...
    voice_connector = {'voiceConnectorId': voice_connector_id,
...
def associate_phone_number(voice_connector, phoneNumber):
    print('Associating Phone Number: {} with Voice Connector {}'.format(
        phoneNumber, voice_connector))
