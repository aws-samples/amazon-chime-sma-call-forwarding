...
const App = () => {
    ...
    const handleSubmitForward = async (event) => {
        ...
        if (isValidPhoneNumber(forwardToNumberValue)) {
            ...
            try {
                const phoneNumberResponse = await API.post('callControlAPI', 'updateNumber', {
                    body,
                });
                if (phoneNumberResponse.status === 200) {
                    alert(forwardNumberValue.value + ' has been forwarded to ' + forwardToNumberValue);
                    setServiceOptionValue(0);
            ...
            alert(forwardToNumberValue + ' is NOT a valid phone number.  Please enter a valid E.164 number');
            setForwardToNumberValue('');
        }
    };
            ...
            if (phoneNumberResponse.status === 200) {
                alert(
                    'Forward on ' +
                        ...
                        ' has been removed and number assigned to Voice Connector' +
                        voiceConnectorValue.value,
                );
                setServiceOptionValue(0);
                setVoiceConnectorValue();
                setRemoveForwardNumberValue(0);
    ...
    const forwardableNumbers = phoneNumbers
        ...
        .map((number) => {
            ...
            forwardableNumbersOptions.label =
                number.E164PhoneNumber + ' - ' + number.ProductType + ' - ' + number.Status;
    ...
    useEffect(() => {
        const fetchVoiceConnectors = async () => {
            const body = {};
            try {
                const voiceConnectorResponse = await API.post('callControlAPI', 'listVoiceConnectors', {
                    body,
                });
                setVoiceConnectors(voiceConnectorResponse.VoiceConnectors);
            } catch (error) {
        ...
        const fetchPhoneNumbers = async () => {
            const body = {};
            try {
                const phoneNumberResponse = await API.post('callControlAPI', 'queryNumber', {
                    body,
                });
                setPhoneNumbers(phoneNumberResponse.PhoneNumbers);
            } catch (error) {
                console.log('error', error);
            }
        };
    ...
    return (
        <div>
            ...
            <Select value={serviceOptionValue} onChange={handleServiceSelection} options={serviceOptions} isClearable />
            <div>
                <div>
